...
class ProcessingService : Service() {
    ...
    data class TaskResult(val success: Boolean, val notificationText: String)
    ...
    companion object {
        ...
        private var pendingTask: (suspend CoroutineScope.() -> TaskResult)? = null
        ...
        fun start(context: Context, task: suspend CoroutineScope.() -> TaskResult) {
            synchronized(taskLock) {
                check(pendingTask == null && !isRunning) { "已有视频处理任务正在运行" }
                pendingTask = task
            }
            try {
                ...
                synchronized(taskLock) { pendingTask = null }
                throw e
            }
        }
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        serviceScope.launch {
            ...
            complete(result.success, result.notificationText)
        }
        return START_NOT_STICKY
    }
