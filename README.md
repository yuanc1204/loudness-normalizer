...
进入 [Releases](https://github.com/yuanc1204/loudness-normalizer/releases/latest)，下载其中的 APK 文件并安装。
如果系统提示不允许安装，请按提示允许浏览器或文件管理器“安装未知应用”。
...
- 可在视频编辑页快速选取成品视频封面；仅改封面时不重编码整段画面
- 点击视频标题可修改生成视频的名称
...
1. 从 [Releases](https://github.com/yuanc1204/loudness-normalizer/releases/latest) 下载 APK，安装后打开「响度均衡」。
2. 点「选择视频或音频」，也可以从 Telegram 等应用直接分享文件到本应用。
3. 按需调整目标响度和均衡力度；一般保持默认值即可。
4. 视频可以点预览图裁剪、选择当前画面作为成品封面，也可以选择多个视频排序、拼接。
...
需要 JDK 17 和 Android SDK 34。在 `android` 目录运行：
...
1. 点击仓库页面右上方 `Code`，选择 `Download ZIP` 并解压。
2. 安装 [Python](https://www.python.org/downloads/) 和 [FFmpeg](https://ffmpeg.org/download.html)，安装时确保它们可从命令行运行。
3. 把一个或多个视频拖到 `响度均衡.bat` 上。
4. 在仓库根目录的 `输出` 文件夹中查看成品；原视频不会被修改。
...
本项目代码采用 [MIT License](LICENSE)，内置依赖仍遵循各自许可证，见 [第三方许可说明](THIRD_PARTY_NOTICES.md)。
